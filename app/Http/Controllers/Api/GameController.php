...
class GameController extends Controller
{
    ...
    public function nextThreeMatches()
    {
        try {
            ...
            $data['matches'] = GameResource::collection($matches);
    ...
    public function nextMatch()
    {
        try {
            $currentDate = Carbon::now()->format('Y-m-d-h-i');
            $nextMatch = Game::where('when', '>=', $currentDate)->where('status', 'not_started')->limit(3)->get();
    ...
    public function lastMatchDetails()
    {
        try {
            ...
            $data['score'] = $this->getMatchScore($lastMatch->uuid);
            return $this->apiResponse($data);
        } catch (\Throwable $th) {
            return $this->errorResponse('the match in unavailable : ' . $th->getMessage(), 404);
        }
    ...
    public function getMatchByDate($date) //the date format must be "Y-m-d"
    {
        ...
        try {
            ...
            foreach ($matches as $match) {
                ...
                if ($date == $match->when) {
                    $data['match details'] = [MatchByDateResource::make($match), $this->getMatchScore($match->uuid)];
